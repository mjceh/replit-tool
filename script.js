...
// Smart authentication using stored credentials only
async function tryAutoAuthentication() {
    try {
        ...
        showToast('🔍 Checking for saved login...', 'info');
        authStatusText.textContent = 'Looking for saved credentials...';
        authSpinner.style.display = 'inline-block';
        
        const storedCreds = localStorage.getItem('fb_auto_credentials');
        if (storedCreds) {
            try {
                const authData = JSON.parse(storedCreds);
                
                // Check if credentials are still fresh (less than 24 hours)
                const now = Date.now();
                const storedTime = authData.timestamp || 0;
                const hoursSinceStored = (now - storedTime) / (1000 * 60 * 60);
                
                if (hoursSinceStored < 24) {
                    const success = await handleAuthSuccess(authData);
                    if (success) {
                        showToast('✅ Using saved login!', 'success');
                        return true;
                    }
                } else {
                    // Credentials too old, remove them
                    localStorage.removeItem('fb_auto_credentials');
                    showToast('🕒 Saved login expired (24h limit)', 'info');
                }
            } catch (e) {
...
async function checkAuthStatus() {
    try {
        ...
        if (isAuthenticated) {
            ...
            const autoAuthSuccess = await tryAutoAuthentication();
            if (!autoAuthSuccess) {
                showLoginInterface();
            }
        }
...
function showLoginInterface() {
    hideMainContent();
    authStatusText.textContent = 'Please connect your Facebook account';
    authSpinner.style.display = 'none';
    showToast('🔐 Please log in to Facebook to continue', 'info');
}
...
async function handleAuthSuccess(authData) {
    try {
        ...
        if (result.status === 'success') {
            ...
            // Hide popup if exists and update UI
            ...
            // Store credentials for future auto-detection with timestamp
            try {
                const credentialsWithTimestamp = {
                    ...authData,
                    timestamp: Date.now()
                };
                localStorage.setItem('fb_auto_credentials', JSON.stringify(credentialsWithTimestamp));
            } catch (e) {
...
document.getElementById('setupBookmarkletButton').addEventListener('click', handleSetupBookmarklet);
logoutButton.addEventListener('click', handleLogout);

// Handle auto-detect button click
async function handleAutoDetect() {
    showToast('🔍 Checking for saved Facebook login...', 'info');
    const success = await tryAutoAuthentication();
    if (!success) {
        showToast('❌ No saved login found. Please setup one-click login first.', 'warning');
    }
}

// Handle bookmarklet setup
async function handleSetupBookmarklet() {
    try {
        showToast('⚡ Setting up one-click login...', 'info');
        
        // Fetch the bookmarklet from server
        const response = await fetch('/bookmarklet');
        const data = await response.json();
        ...
        if (data.bookmarklet) {
            ...
            copyBtn.onclick = () => copyBookmarklet(data.bookmarklet);
            
            showToast('✅ One-click login is ready! Follow the steps above.', 'success');
            
            // Start checking for credentials from bookmarklet
            startCredentialListener();
        } else {
            showToast('❌ Failed to setup one-click login', 'error');
        }
    } catch (error) {
        console.error('Error setting up bookmarklet:', error);
        showToast('❌ Error setting up one-click login', 'error');
    }
}

// Copy bookmarklet to clipboard
async function copyBookmarklet(bookmarkletCode) {
    try {
        await navigator.clipboard.writeText(bookmarkletCode);
        showToast('📋 Bookmarklet copied! Save it as a bookmark.', 'success');
    } catch (error) {
...
// Listen for credentials from bookmarklet
    ...
    const checkInterval = setInterval(async () => {
        try {
            ...
            if (data.authenticated) {
                ...
                const authData = {
                    fb_dtsg: 'from_bookmarklet',
                    cookie_header: 'from_bookmarklet', 
                    av_value: 'from_bookmarklet',
                    user_name: data.user_name,
                    timestamp: Date.now()
                };
                
                try {
                    localStorage.setItem('fb_auto_credentials', JSON.stringify(authData));
                } catch (e) {
                    console.log('Could not store credentials locally');
                }
                
                showToast(`🎉 Successfully connected as ${userName}!`, 'success');
